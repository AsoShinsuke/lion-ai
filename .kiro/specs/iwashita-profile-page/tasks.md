...
- [ ]* 6.2 プロパティテスト: すべての外部リンクは有効なURL形式を持つ
...
- [ ]* 7.1 Footerコンポーネントのユニットテスト
  - 著作権表示が正しく表示されることを検証

- [ ] 8. メインページの実装（app/page.tsx）
  - すべてのコンポーネントを統合
  - プロフィールデータを各コンポーネントに渡す
  - セマンティックHTML構造（header、main、footer）を使用
  - _要件: 7.3_

- [ ]* 8.1 プロパティテスト: HTMLはセマンティック構造を持つ
...
- [ ]* 9.1 プロパティテスト: 必須メタデータが存在する
  - **プロパティ 4: 必須メタデータが存在する**
  - **検証: 要件 7.1, 7.2**

- [ ] 10. グローバルスタイルとTailwind設定
...
- [ ]* 10.1 プロパティテスト: レスポンシブデザインのメディアクエリが存在する
...
- [ ]* 11.1 プロパティテスト: すべての画像は代替テキストを持つ
